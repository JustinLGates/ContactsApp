...
function drawContacts() {
  ...
  contacts.forEach((contact) => {
    contactTemplate += `<div class="card mt-1 mb-1 ${
      contact.emergency ? "emergency-contact" : ""
       ...
       <i onclick="removeContact(${
         contact.id
       })" class="action fa fa-trash text-danger"></i>
...
function removeContact(contactId) {
  console.log("if " + contactId);
  let index = contacts.findIndex((contact) => contact.id == contactId);
  console.log("is equal to " + contacts[0].id);
  if (index <= -1) {
    throw new Error(
